...
class SwinIR(nn.Module):
    ...
    def __init__(self, in_chans=3, upscale=4, num_feats=180, depths=[6, 6, 6, 6, 6, 6], num_heads=[6, 6, 6, 6, 6, 6],
                 window_size=8, train_resolution=(48, 96), mlp_ratio=2., qkv_bias=True, qk_scale=None,
                 attn_drop_rate=0., drop_rate=0., drop_path_rate=0.1, norm_layer=nn.LayerNorm,
                 use_mask=True, use_checkpoint=False):
        ...
        self.tail = nn.Sequential(nn.Conv2d(num_feats, 64, 3, 1, 1),
    ...
    def forward(self, x):
        ...
        x =  x_rec[:, :, :H*self.upscale, :W*self.upscale]
        # x = torch.clamp(x, 0, 1)
        return x
...
if __name__ == '__main__':
    ...
    model.eval()
    B, C, H, W = 16, 3, 77, 256
    lr = torch.rand((B, C, H, W)).to(device)
